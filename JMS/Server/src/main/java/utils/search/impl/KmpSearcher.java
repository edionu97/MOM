...
public class KmpSearcher implements ISearcher {
    ...
    @Override
    public int getNumberOfApparitions(final String searchableString, final String pattern) {
    ...
    @Override
    public boolean isIn(final String pattern, final String parent) {
        ...
        for (int i = 0; i < parent.length(); ++i) {
            ...
            // compare character by character the pattern and the string
            while (k > 0 && parent.charAt(i) != pattern.charAt(k)) {
                k = next[k - 1];
            }

            //if the characters are equal than continue
            if (parent.charAt(i) == pattern.charAt(k)) {
                ++k;
            }
